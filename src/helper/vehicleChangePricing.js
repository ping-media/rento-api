...
const calculateVehicleChangePricing = async (
  ...
  console.log("paidExtensions count:", paidExtensions.length);
  console.log("now:", now);
  ...
  // DEBUG
  console.log("currentSegment:", JSON.stringify(currentSegment, null, 2));
  ...
  const lastRelevantVehicleChange = booking.bookingPrice.diffAmount
    ?.filter((d) => {
      ...
      // New entries: flagged explicitly
      if (d.isSameMasterSwap) return false;
      // Old entries without flag: same-model swaps and zero-days swaps
      // always have oldAmount=0 and newAmount=0 — skip them as baselines
      if (d.newAmount === 0 && d.oldAmount === 0) return false;
  ...
  console.log(
    "all diffAmount entries:",
    JSON.stringify(
      booking.bookingPrice.diffAmount?.map((d) => ({
        title: d.title,
        isSameMasterSwap: d.isSameMasterSwap,
        paymentInitiatedDate: d.paymentInitiatedDate,
        newVehicleSnapshot: d.newVehicleSnapshot,
      })),
      null,
      2,
    ),
  ...
  console.log(
    "lastRelevantVehicleChange:",
    JSON.stringify(lastRelevantVehicleChange, null, 2),
  );
  console.log(
    "currentSegment.oldCost after snapshot check:",
    currentSegment.oldCost,
  );
  console.log("addonAlreadyInOldCost:", addonAlreadyInOldCost);
  ...
  // DEBUG
  console.log("segmentDays:", segmentDays);
  console.log("rawDaysLeft:", rawDaysLeft);
  ...
  // DEBUG
  console.log("oldRemainingValue:", oldRemainingValue);
  console.log("newRemainingCost:", newRemainingCost);
