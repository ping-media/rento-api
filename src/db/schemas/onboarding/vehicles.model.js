...
async function createVehicle({
  ...
  if (result) {
    ...
    BookingStartDateAndTime
      ? (updateObj.BookingStartDateAndTime = BookingStartDateAndTime)
      : "";
    BookingEndDateAndTime
      ? (updateObj.BookingEndDateAndTime = BookingEndDateAndTime)
      : "";
    ...
    const bookingObj = {
      bookingDuration: bookingDurationObj,
      vehicleNumber,
      BookingStartDateAndTime,
      BookingEndDateAndTime,
      isBooked: false,
    ...
    if (result) {
      ...
      if (checkStations.length) {
        for (let i = 0; i < checkStations.length; i++) {
          const find = stations.find(
            (ele) => ele.stationName === checkStations[i].station.value,
          );
          if (!find) {
            checkStation = false;
            break;
